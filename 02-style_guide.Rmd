...
rM_1.r
```
When creating a file, it is important to choose a good name. The same 
applies for the names of directories into files are organised. Changing the name 
or location of a file later can cause a number of problems:  
...
* Sometimes, there are restrictions in place for file names (e.g. the package 
  ...
  testing start with "test" and reside in a particular directory).
* Use expressive but concise names. Describe the function of the file 
...
* Use all lower case names for functions. One motivation for this is that it can 
  save keystrokes and improve typing efficiency. Possibly more important though 
  is that - given that R is case-sensitive - all lower-case function names will 
  be easier for other R developers and users (as well as you yourself) to recall 
  when wanting to make use of a function. Instead of using camelCase, separate 
...
distinguished with ease from other contexts where an equals sign may be used 
e.g. defining arguments within a function call, naming elements within a list, 
...
Some of the reasons outlined here are taken from
[this](http://programmers.stackexchange.com/questions/604/is-the-80-character-
limit-still-relevant-in-times-of-widescreen-monitors) stackoverflow
question.]

* On a typical screen, the code typically gets displayed only in one tab or 
  window, so space is limited. Since you will always have lines with very 
  little code, it is a waste of screen space to have large differences in the 
  line width.
* Line-by-line diffs in a version control system are more legible if lines are 
  not too long.
* It is rare, but if code needs to be printed, endless lines are a nightmare.


## Commenting code
Commenting code in a broad sense is structuring code. In a narrow sense, it 
is about explaining *why* something is done the was it is done, rarely the 
*how* also needs to be explained. 
\
Comments always start with `#` in R. For three levels of granularity, 
we can use `###`, `##` and `#`.

### Structuring code
To structure code, it can be grouped into segments, sections and subsections. 
We recommend starting with sections only, so as your code base grows, you can 
extend in both directions of granularity. However, since you don't want to 
take it too far with commenting, three levels will hardly be necessary in most
cases. Also note that segments use roman numbers, sections arabic numbers and 
subsections letters.  The idea is that segments are represented by the thickest 
*lines* (since they represent the top level) and subsections are represented by 
*loose lines*. 
```{r}
# good
### ============================================================================
...
We discourage the use of plain `#` for a line of comment with the title of the 
section centered.
```{r}
# bad
############################ A TITLE GOES HERE #################################
...
* `#` is rather dominant and has an irregular pattern, which is likely to cause
  distraction to the eye.
* Putting the title in the same line as the separator has two main
  disadvantages: For every separator, the width of the `#`s has to be adjusted 
  plus it is tedious to get the title exactly centered. Also, no code is 
  centered, so why should these markers be?
...
```
If code is not extremely complex, the user wants to know *why* someone does something, 
not *how* (because he can read the code). Often, it makes sense to organize 
comments in blocks instead of commenting each and every line.
If you are writing a function and its inputs and outputs need to be explained
in more depth, create a package for it and use the `roxygen2` package^[For more 
information on the package, check out the [vignette](https://cran.r-project.org/
web/packages/roxygen2/vignettes/roxygen2.html).] to generate 
help files from a comment block at the top of your function. Comments should be 
put next to the function calls on the same line. If they do not fit there fully, 
move them to the line above the code
...
```
In line with the previous subsection, we recommend having a set of three levels
of granularity available for explaining code. Start with `##` so you have an 
option for both lower level and higher level comments left. 
...
specific functions you want to use are functionals or the pipe. In more advanced
settings, you might want to resort to *recursive functions*.^[See an example
...
with functionals. However, not all nested code can be replaced with functionals. 
We do not want to dive into that further here, since Hadley 
...
context of good style. The task is to return the data types of each column 
in the data frame `mtcars`.
```{r}
# good
lapply(mtcars, class)
# bad
...
On a related note, we should mention the pipe (`%>%`), a function that was
introduced in the `magrittr` package. It allows us to chain multiple operations
instead of nesting them, which significantly improves readibilty, See the 
following example provided in the [package vignette](https://cran.r-project.org/web/packages/magrittr/vignettes/magrittr.html). 
The following two code blocks evaluate to the same result.
```{r}
# good
library(magrittr)
car_data <- 
...
```
Formally, the pipe is defined as `x %>% f(y) = f(x, y)`, that is, the first 
argument of a function following the pipe is always the output of the statement
before the pipe. Hence, the verbal interpretation is best expressed with the 
word *then*. In the above case: 
take mtcars, *then* subset it with hp > 100, *then*, aggregate the data, *then* 
transform it, and *then* return it. Also, the pipe can help you to avoid 
creating a lot of throwaway variables on the fly that you don't need later on. The 
following two examples result in the same output.
```{r}
scope <- 1:10
# good
max_of_min <-
...
```
In this example, we only have one argument for the function `min`,
which is transfered with the pipe, so there is no need to supply any argument. 
Of course, this example is trivial but it generalizes to more complex situations.
...
Boolean values `TRUE` and `FALSE` can be abreviated with `T` and `F`, but it is 
bad practice since it is harder to spot them.
```{r, eval = FALSE}
# good
if (cond == TRUE) xyz()
# bad
if (cond = F) xyz()
```
Boolean conditional statements can be abbreviated in the sense that they do not
require a comparison counterpart. The following two
*if statements* are identical, but the first is more explicit.
```{r, eval = FALSE}
# adviced
if (cond == FALSE){
  # do xyz
...
statements. Writing `all(boolean) == TRUE)` might worsen 
legibility, so we drop the comparison.
```{r}
...
```
In some situations, booleans allow you to do away completely with conditional
statements. Whether tend to advocate for this since the amount of lines used 
with this approach is four times lower.
```{r}
y = 0
k = 1
boolean = TRUE
...
In principal, R lets you write a whole *if-else-statement* in one line. 
That is five times less than using the verbose form. In the light of 
these  relations, it might be ok to use the *one-liner* for very short statements.
However, when the situation gets more complex, using a verbose form is adviced 
to maintain code which is well understandable.
...
An alternative to `ifelse` is the function `switch()` if the 
...
condition does not match with any option given. Also, note that switch is not 
particularly flexible since it uses non-standard evaluation and cannot handle 
numeric input.^[i.e. replacing *fail* below with the scalar 1 would raise an error]
...
rm(msg)
```
We do not claim that the warpping the conditional statement into a function like
`paste` in every siutuation, but in the above case, it seems to make the code 
more concise while not introducing confusion.

### Formulation of conditions
The very same conditions can be formulated in different ways. There are some 
basic notions that are worth following.
For example, note that double negation is not desirable in conditional 
statements since it is quite confusing in some cases, although
it technically works fine. Also, it seems quite natural to put a *true* condition
...
Alternatively, you might want to define the variable `success = !fail` 
...
```
Obviously, this only works for simple cases where values are asigned to only 
one variable. It is advised in simple cases because it becomes evident very 
quickly that the whole point of this conditional statement is assigning a value
to `x`.
...
```
In other words, when all values in the 
...
Also, lazy evaluation^[note that `&&` and `||` are the lazy equivalents of 
`&` and `|`, i.e. using `&` in the below example would raise an error because 
the second statement cannot be evaluated. With `&&` it does not get evaluated 
because the first statement returns `FALSE`] can be useful in combination with 
conditional statements.
...
place. Here are two ways to deal with that situation.^[If not used in the 
context of a function, you might use `exists("x")` instead
of `missing(x)`]
```{r, eval = FALSE}
# good 
if (!missing(x) && x > 10) {
