...
* processes that take the file name as an input and return an output that is 
  related to this name are problematic. For example, when creating a html file 
  of an Rmd file via `knittr()`, the created object has the same name (but a 
...
* Sometimes, there are restrictions in place for file names (i.e. the package 
  `testthat` used in package development requires all files with test to start 
  with "test" and live in a particular folder).
* Do use expressive but concise names. Describe the function of the file 
  (`clean-temperatur_data.R`), what it inputs or outputs 
...
* Do not use version numbers in the file name. Use a version control system 
  like git instead.
* Use all lower case names. It saves typing. Separate elements of the name with 
  an underscore.
* It can be advantageous to use a combination of numers and descriptive words as 
  a file name. This allows you to a) select files efficiently with bash or 
...
* Choose unique file names, even accross directories. This avoids confusion and
...
* Always use the same spelling of extensions. Although `xzy.r` and `xyz.R` 
  are both recognized as R files, we recommend the latter.

To sum it up, we here are a couple of bad examples for the cases described 
above.
```{r, eval = FALSE}
# synonyms
quick_data_check.R 
...
```
A lot of rules that described in the above section apply for naming objects (in 
the sense of bindings within namespaces) as well. In particular we would like 
